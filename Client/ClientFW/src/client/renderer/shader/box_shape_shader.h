...
namespace client_fw
{
	class BoxShapeShader final : public ShapeShader
	{
	public:
		BoxShapeShader(const std::string& name);
		virtual ~BoxShapeShader() = default;
		...
		//virtual D3D12_SHADER_BYTECODE CreateGeometryShader(ID3DBlob** shader_blob, eRenderLevelType level_type, int pos_index) const override;
