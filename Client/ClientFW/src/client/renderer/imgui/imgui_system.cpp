...
namespace client_fw
{
	...
	bool ImGuiSystem::Initialize(ID3D12Device* device, HWND hwnd)
	{
		IMGUI_CHECKVERSION();
		ImGui::CreateContext();
		...
		{
			LOG_ERROR("Could not initialize imgui Win32");
			return false;
		}
	...
	void ImGuiSystem::Update(ID3D12Device* device)
	{
	...
	void ImGuiSystem::PreDraw(ID3D12GraphicsCommandList* command_list) const
	{
		ImGui::Render();
	}

	void ImGuiSystem::Draw(ID3D12GraphicsCommandList* command_list) const
	{
		ImGui_ImplDX12_RenderDrawData(ImGui::GetDrawData(), command_list);
	}

	void ImGuiSystem::SytleForClientFW()
	{
